...
const rollTheDices = (rolls) => {
  let arrayOfNumbers = { sum: 0, values: [] };

  for (let i = 0; i < rolls; i++) {
    let rollResult = dice();
    console.log("roll:", rollResult);
    arrayOfNumbers.sum += rollResult;
    arrayOfNumbers.values.push(rollResult);
  }
...
function howManyDays(date) {
  ...
  let differenceInTime = now.getTime() - inputDate.getTime();
...
const searchByTitle = (movies, title) => {
  ...
  return movies.find(movie => movie.Title.toLowerCase() === lowerCaseTitle);
}
/*const searchByTitle = (movies, Title) => {
  return movies.find(movie => movie.Title === Title);
}*/

console.log(searchByTitle(movies, 'Lord of the Flies')); 

/* ESERCIZIO 18
  Scrivi una funzione chiamata "searchAndDivide" che riceve una stringa come parametro e ritorna un oggetto contenente due array: "match" e "unmatch".
  "match" deve includere tutti i film dell'array "movies" fornito che contengono la stringa fornita all'interno del proprio titolo, mentre "unmatch" deve includere tutti i rimanenti.
*/
const searchAndDivide = (search, movies) => {
  const match = [];
  const unmatch = [];
...
console.log(searchAndDivide('1', movies));
